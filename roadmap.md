...
Esta página mostra **o estado de cada funcionalidade** do RIT360 Financeiro — o que já está em produção, o que está sendo desenvolvido agora, o que está planejado para mais à frente, e o que só será implementado **se houver demanda**.

> 💡 **Por que esta página existe**
>
> Toda OSC tem necessidades diferentes. Antes de adotar o sistema ou planejar como vai usar, dá para conferir aqui se a funcionalidade que você precisa **já está pronta, está perto ou ainda vai demorar** — sem suposição. Se algo importante para a sua OSC não está aqui, escreva pelo botão **💬 Feedback** dentro do RIT360 Financeiro; ouvimos.
...
>
> Clique no botão **FEEDBACK** em qualquer tela do RIT360 Financeiro — sua mensagem chega direto à equipe da RIT e pode acelerar a priorização de algo que você precisa.

---

## 💸 Financeiro (Movimentações, Painel e Relatórios)

| Funcionalidade | Descrição | Disponível | Em Construção | Planejado | Sob Demanda |
|---|---|:---:|:---:|:---:|:---:|
...
> Tema novo, planejado como uma **onda dedicada na V2** do RIT360 Financeiro — gestão de compras com cotação de fornecedores antes da autorização do pagamento.
...
| Execução (tarefas em Kanban, comunicação, status updates, evidências) | Quadro de tarefas, linha do tempo com comentários/menções e anexos de evidência | ✓ | | | |
...
| Acervo de Lições Aprendidas da organização | Lições e boas práticas dos encerramentos reunidas em um só lugar, com busca e filtros, e sugeridas ao criar um novo projeto | ✓ | | | |
| Relatórios filtrados por projeto | Recorte de receitas, despesas e gráficos por iniciativa | ✓ | | | |
| Templates de projeto reutilizáveis | Modelos pré-configurados por tipo (ex: "Evento", "Obra"), com marcos, tarefas e riscos sugeridos | ✓ | | | |
| Cronograma visual (Gantt) | Visualização gráfica de tarefas e marcos no tempo | | | ✓ | |
| Templates de projeto personalizados pela OSC | Cada OSC cria seus próprios modelos do zero ou salva um projeto existente como template | | | | ✓ |
...
| Notificações granulares por evento e canal | Matriz 10 eventos × 2 canais (e-mail e Push) com switches individuais | ✓ | | | |
| Notificações push via Web Push (PWA) | Push como segundo canal da matriz; ativação por dispositivo via master switch; suporte Android Chrome, iOS Safari (com PWA instalado) e desktop | ✓ | | | |
...
| Verificação de e-mail por código no autocadastro | Código enviado por e-mail confirma a posse do endereço antes de criar a conta | ✓ | | | |
...
| Splash screen com identidade da marca | Tela de carregamento em Verde Teal com símbolo do RIT360 Financeiro ao abrir o app instalado | ✓ | | | |
...
| Tela offline amigável | Quando sem conexão, app exibe tela em pt-BR com logo do RIT360 Financeiro e botão "Tentar novamente" — em vez de erro do navegador | ✓ | | | |
...
| Aplicativo nativo nas lojas Google Play e App Store (iOS) | Distribuição de um app nativo nas lojas oficiais, além do PWA instalável atual | | | ✓ | |
...
- **Botão 💬 Feedback** dentro do RIT360 Financeiro — sua mensagem é lida pela equipe da RIT e pode acelerar a priorização de algo que você precisa.
